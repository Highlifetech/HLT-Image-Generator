...
def build_lifestyle_prompt(scene, model_type, custom_prompt):
    scene_descriptions = {
        'street': 'urban street style setting, city sidewalk, natural daylight',
        'studio': 'professional photography studio, clean backdrop, studio lighting',
        'outdoor': 'beautiful outdoor natural setting, golden hour lighting',
        'urban': 'trendy urban environment, graffiti walls, industrial chic',
        'beach': 'sunny beach setting, ocean waves, sandy shore, tropical vibes',
        'rooftop': 'stylish rooftop terrace, city skyline view, sunset lighting',
        'coffee': 'cozy coffee shop interior, warm ambient lighting',
        'gym': 'modern fitness studio, athletic environment, dynamic lighting',
    }
    model_descriptions = {
    ...
    prompt = (
        ...
        f"The product must be clearly visible and accurately represented. "
        ...
        f"Do not add text or watermarks."
    )
    return prompt
...
def generate():
    ...
    for filename in generated_files:
        images.append({'url': f'/images/{filename}', 'filename': filename})
    gallery_entry = {
        'id': uuid.uuid4().hex,
...
def download_all():
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name='hlt_lifestyle_images.zip',
    )
